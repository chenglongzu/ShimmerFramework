...
namespace ShimmerFramework
{
	...
	public class SocketTcpRoutine
	{
		...
		#region 接收消息所需变量
		//接收数据包的缓冲数据流
		private ShimmerMemoryStream m_ReceiveMS = new ShimmerMemoryStream();
		//接收数据包的字节数组缓冲区
		...
		/// 发送数据的MemoryStream
		/// </summary>
		private ShimmerMemoryStream m_SocketSendMS = new ShimmerMemoryStream();
		/// <summary>
		/// 接收数据的MemoryStream
		/// </summary>
		private ShimmerMemoryStream m_SocketReceiveMS = new ShimmerMemoryStream();
		...
		private void ConnectCallBack(IAsyncResult ar)
		{
			...
			{
				SocketManager.GetInstance().RegisterSocketTcpRoutine(this);

				ReceiveMsg();

				Debug.Log(string.Format("Socket连接到=={0}==服务器成功!", m_Client.RemoteEndPoint));
				OnConnectOK?.Invoke();
			}
			else
			{
		...
		public void DisConnect()
		{
			...
			{
				Debug.Log(string.Format("Socket从=={0}==服务器断开连接!==", m_Client.RemoteEndPoint));
				m_Client.Shutdown(SocketShutdown.Both);
				m_Client.Close();
				SocketManager.GetInstance().RemoveSocketTcpRoutine(this);
			}
		}
		#endregion

		#region CheckSendQueue 检查接收队列
		private void CheckReceiveQueue()
        {
			while (true)
			{
				if (m_ReceiveCount <= SocketManager.GetInstance().MaxReceiveCount)
				{
					m_ReceiveCount++;
					lock (m_ReceiveQueue)
					{
						if (m_ReceiveQueue.Count > 0)
						{
							...
							//1.解析队列中的数据包
							ShimmerMemoryStream ms1 = m_SocketReceiveMS;
							ms1.SetLength(0);
							...
							ShimmerMemoryStream ms2 = m_SocketReceiveMS;
							ms2.SetLength(0);
							...
							EventManager.GetInstance().GlobalEventTrigger(protoCode, protoContent);
						}
						else
						{
							break;
		...
		{
			if (m_SendCount >= SocketManager.GetInstance().MaxSendCount)
			{
				//等待下一帧发送
				m_SendCount = 0;
				return;
			...
			lock (m_SendQueue)
			{
				...
				{
					ShimmerMemoryStream ms = m_SocketSendMS;
					ms.SetLength(0);
					...
					while (true)
					{
						...
						if (buffer.Length + ms.Length <= SocketManager.GetInstance().MaxSendByteCount)
						{
